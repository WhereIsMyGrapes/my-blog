<template>
  <div class="reply-warp" id="reply-wrap">
    ...
    <ReplyBox @reload="reloadComments" :comment-type="commentType" :type-id="typeId" />
    <div v-if="count > 0 && reFresh">
        ...
        <div class="content-warp">
          ...
          <div class="reply-info">
            ...
            <span class="reply-like" @click="like(comment)">
              <svg-icon icon-class="like" class="like" size="0.8rem"
               :class="isLike(comment.id)" style="margin-right: 5px" />
          ...
          <div class="sub-reply-item" v-for="reply of comment.replyVOList" :key="reply.id">
            ...
            <span class="reply-content">
              <!-- 不是自己回复自己, 就显示"回复 @xxx(被回复人) :"字样 -->
              <template v-if="reply.fromUid !== reply.toUid">
                回复
                <span style="color: #0085ac5">@{{ reply.toNickname }}</span>
                :
              </template>
      ...
      <!-- 查看更多 -->
      <div class="loading-warp" style="margin-bottom: 10px" v-if="count > commentList.length">
        <n-button class="btn" color="#e9546b" @click="getList">
          加载更多...
        </n-button>
      </div>
    </div>
    <div v-else style="padding: 1.25rem; text-align: center">评论区空空如也~</div>
  </div>
</template>
...
const handleReply = (index: number, target: Comment | Reply) => {
  console.log("fromUid", commentList.value[index]);
