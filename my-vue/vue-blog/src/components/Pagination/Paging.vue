<template>
  <div class="view-more-pagination" v-show="show">
    ...
    <span class="patination-btn" v-if="current !== 1" @click="prePage">上一页</span>
    <template v-for="(number, index) in visibleNumber">
      <span :key="index" @click="changePage(number)" v-if="typeof number == 'number'" 
        class="pagination-page-number" :class="current == number ? 'current-page' : ''">
        {{ number }}
      </span>
...
const visibleNumber = computed(() => {
  if (totalPage.value <= 5) {
    ...
    //   return [1, "...", totalPage.value - 5, totalPage.value - 4, totalPage.value - 3, totalPage.value - 2, totalPage.value - 1, totalPage.value];
      ...
      return [1, '...', ...adjacentPages, '...', totalPage.value] 
    //   return [
    //     1,
    //     "...",
    //     current.value - 2,
